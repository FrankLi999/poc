...
public class JobAnnouncementProcessTest extends BaseProcessWithMockTestCase {
	...
	@Test
	@Deployment(resources = { "camunda-testing-job-announcement.bpmn",
	...
	public void testHappyPath() {
		...
		assertThat(processInstance).isStarted().isNotEnded().task().hasDefinitionKey("edit")
				.hasCandidateGroup("engineering").isNotAssigned();
		...
		assertThat(processInstance).variables().hasSize(2).containsEntry("jobAnnouncementId", jobAnnouncement.getId())
		...
		assertThat(processInstance).hasPassedInOrder(findId("Stelle beschreiben"),
				findId("Stellenbeschreibung sichten"), findId("Ausschreibung anstoßen"),
				findId("Durchführung der Ausschreibung"), findId("Erfolgsmeldung senden"));
