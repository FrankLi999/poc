...
public class OrderProcessCoverageTest extends BaseProcessWithMockTestCase {
	...
	public void setup() throws Exception {
		...
		when(orderProcess.waitsAtReceiveTask(anyString()))
	...
	public void testOrderHappyPath() throws Exception {
		...
		StartingByStarter starter = Scenario.run(orderProcess) //
				.startBy(() -> {
					...
					// as this might do other things, foremost data transformation and naming of
					// process variables
					return orderRestController.placeOrder(orderId, 547);
				});

		// expect the charge for retrieving payments to be created correctly and return
		// a dummy transactionId
		mockRestServer.expect(ExpectedCount.manyTimes(), requestTo("http://api.test.org:80/payment/charges")) //
				// .expect(requestTo("/payment/charges")) //
				.andExpect(method(HttpMethod.POST)).andExpect(jsonPath("amount").value("547"))
		...
		when(orderProcess.waitsAtReceiveTask("ReceiveTask_WaitForGoodsShipped")).thenReturn((messageSubscription) -> {
			...
			// again: use the real thing like receiving the real AMQP message, return a
			// dummy shipmentId
		...
		assertThat(scenario.instance(orderProcess)).variables()
				.containsEntry(ProcessConstants.VARIABLE_paymentTransactionId, "12345");
		assertThat(scenario.instance(orderProcess)).variables().containsEntry(ProcessConstants.VAR_NAME_shipmentId,
				"0815");

		{
			ArgumentCaptor<Message> argument = ArgumentCaptor.forClass(Message.class);
			verify(rabbitTemplate, times(1)).convertAndSend(eq("shipping"), eq("createShipment"), argument.capture());
			// if the body would be an object, JSON or whatever, you can easily
			// inspect/assert it here in detail
