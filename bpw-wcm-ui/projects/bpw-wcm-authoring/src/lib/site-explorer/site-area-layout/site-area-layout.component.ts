...
export class SiteAreaLayoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.store
      ...
      .subscribe(wcmSystem => {
        ...
        this.layout =
          this.siteArea.siteAreaLayout != undefined
            ? cloneDeep(this.siteArea.siteAreaLayout)
            : this.emptyLayout();
      });
  ...
  private resolveLayoutFromPageLayout(): SiteAreaLayout {
    let contentAreaLayout = this.siteArea.properties["contentAreaLayout"]
      ? this.contentAreaLayouts[this.siteArea.properties["contentAreaLayout"]]
      : null;
    return {
