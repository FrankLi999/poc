...
export class SiteTreeComponent extends WcmNavigatorComponent
  ...
  ngOnInit() {
    ...
    this.functionMap["Purge.siteArea"] = this.purgeWcmItem;
    this.functionMap["Create.content"] = this.createContent;
    this.functionMap["Edit.content"] = this.editContent;
    this.functionMap["Purge.content"] = this.purgeWcmItem;
  ...
  createFolder() {
    ...
    let dialogRef = this.matDialog.open(NewFolderDialogComponent, {
      ...
      data: {
        jsonFormObject: this.jsonFormMap["bpwizard/default/system/folderType"]
          .formSchema,
        nodePath: node.wcmPath,
        repositoryName: node.repository,
        workspaceName: node.workspace
  ...
  createTheme() {
    ...
    let dialogRef = this.matDialog.open(NewThemeDialogComponent, {
      ...
      data: {
        jsonFormObject: this.jsonFormMap["bpwizard/default/system/themeType"]
          .formSchema,
        nodePath: node.wcmPath,
        repositoryName: node.repository,
        workspaceName: node.workspace
  ...
  removeItem() {
    ...
    this.modeshapeService
      .deleteItem(node.repository, node.workspace, node.wcmPath)
      .subscribe(
        (event: any) => {
          if (event.type === 4) {
            this.nodeRemoved(node);
  ...
  editContent() {
    const node = this.activeNode;
    this.wcmService
      .getContentItem(node.repository, node.workspace, node.wcmPath)
      .subscribe(response => {
