...
export abstract class WcmNavigatorComponent implements OnInit, OnDestroy {
  ...
  @Output() nodeSelected: EventEmitter<
    WcmItemFlatTreeNode
  > = new EventEmitter();
  ...
  private generateLibraryNode(
    ...
    if (!wcmNode) {
      wcmNode = new WcmItemTreeNode(
        libraryPath,
        library,
        repository,
        workspace,
        this.rootNodeType,
        `bpwizard/library/${library}/${this.rootNode}`,
        null
      );
  ...
  private generateWcmNode(
    parent: WcmItemTreeNode,
    wcmNodeName: string,
    repository: string,
    workspace: string,
    nodeType: string,
    ...
    const wcmTreeNode = new WcmItemTreeNode(
      nodePath,
      wcmNodeName,
      repository,
      workspace,
  ...
  getChildren = (node: WcmItemTreeNode): Observable<WcmItemTreeNode[]> =>
    node.childrenChange;
  ...
  loadWcmNode(parent: WcmItemTreeNode, onlyFirstTime = false) {
    ...
    console.log(">>>>>>>> loading tree", parent.wcmPath);
    this.nodeFilter.nodePath = parent.wcmPath;
    this.wcmService
      .getWcmNodes(parent.repository, parent.workspace, this.nodeFilter)
      .subscribe(
        (wcmNodes: WcmNode[]) => {
          if (wcmNodes) {
            const wcmTreeNodes: WcmItemTreeNode[] = wcmNodes.map(wcmNode =>
              this.generateWcmNode(
                parent,
                wcmNode.name,
                parent.repository,
                parent.workspace,
