...
import { RenderTemplate, AuthoringTemplate } from "bpw-wcm-service";
...
export class RenderTemplateComponent extends WcmConfigurableComponent
  ...
  ngOnInit() {
    ...
    this.route.queryParams
      ...
      .subscribe(renderTemplate =>
        this.subscribeRenderTemplate(renderTemplate)
      );
  ...
  getRenderTemplate(param: any): Observable<RenderTemplate> {
    ...
    if (this.editing) {
      return this.wcmService.getRenderTemplate(
        this.repository,
        this.workspace,
        this.nodePath
      );
  ...
  subscribeRenderTemplate(renderTemplate: RenderTemplate) {
    ...
    this.renderTemplateForm.patchValue({
      ...
      selectedContentType: this.renderTemplate.isQuery
        ? ""
        : this.renderTemplate.resourceName,
      selectedQuery: this.renderTemplate.isQuery
        ? this.renderTemplate.resourceName
        : "",
  ...
  hasContentItems(): boolean {
    return (
      this.renderTemplate && this.renderTemplateForm.get("maxEntries").value > 0
    );
  }
  ...
  contentElements(): string[] {
    ...
    return selectedContentType
      ? this.contentElementsMap.get(selectedContentType)
      : [];
  }
