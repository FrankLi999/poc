...
import {
    ComponentModel,
    ValidationModel,
    FormioValidationModel
} from "..";
import { Utils } from "../utils/core.utils";
import { ValidationUtils } from "../utils/validation.utils";

export type FormioValidatorFactory = (args: any) => ValidatorFn | AsyncValidatorFn;

export type FormioValidatorsToken = (ValidatorFn | AsyncValidatorFn)[];

@Injectable()
export class FormioFormValidationService {
    ...
    private getValidatorFn(validatorName: string, validatorArgs: any = null,
                           validatorsToken: FormioValidatorsToken = this.ngValidators): ValidatorFn | AsyncValidatorFn | never {
        let validatorFn: FormioValidatorFactory | ValidatorFn | AsyncValidatorFn | null = null;
    ...
    private getValidatorFns(validatorsModel:  ValidationModel,
                            validatorsToken: FormioValidatorsToken = this.ngValidators): ValidatorFn[] | AsyncValidatorFn[] {

        let validatorsConfig:  {[validatorKey: string]: FormioValidationModel};
        ...
        if (Utils.isTrueObject(validatorsModel)) {
            validatorFns = Object.keys(validatorsModel).filter(validatorFnKey => {
                    ...
                    if (validatorFnKey === 'custom') {
                        validatorName = validatorsModel[validatorFnKey];
                        validatorArgs = null;
                    } else {
            ...
            if (validatorsModel.custom && (!Utils.isString(validatorsModel.custom))) {
                let nameArgs = validatorsModel.custom as Array<FormioValidationModel>;
