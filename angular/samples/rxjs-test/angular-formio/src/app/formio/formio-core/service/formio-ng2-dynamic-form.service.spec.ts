import { TestBed, inject } from "@angular/core/testing";
import {
    ReactiveFormsModule,
    FormGroup,
    FormControl,
    FormArray,
    NG_VALIDATORS,
    NG_ASYNC_VALIDATORS
} from "@angular/forms";
import { FormioNg2DynamicFormService } from "./formio-ng2-dynamic-form.service";
import { FormioNg2DynamicFormValidationService } from "./formio-ng2-dynamic-form-validation.service";
import { FormioCheckboxModel } from "../model/checkbox/formio-checkbox.model";
import { FormioCheckboxGroupModel } from "../model/checkbox/formio-checkbox-group.model";
import { FormioDateControlModel } from "../model/formio-date-control.model";
import { FormioDatePickerModel } from "../model/datepicker/formio-datepicker.model";
import { FormioEditorModel } from "../model/editor/formio-editor.model";
import { FormioFileUploadModel } from "../model/file-upload/formio-file-upload.model";
import { FormioFormArrayModel } from "../model/form-array/formio-form-array.model";
import { FormioFormControlModel } from "../model/formio-form-control.model";
import { FormioFormGroupModel } from "../model/form-group/formio-form-group.model";
import { FormioInputModel } from "../model/input/formio-input.model";
import { FormioRatingModel } from "../model/rating/formio-rating.model";
import { FormioRadioGroupModel } from "../model/radio/formio-radio-group.model";
import { FormioSelectModel } from "../model/select/formio-select.model";
import { FormioSliderModel } from "../model/slider/formio-slider.model";
import { FormioSwitchModel } from "../model/switch/formio-switch.model";
import { FormioTextAreaModel } from "../model/textarea/formio-textarea.model";
import { FormioTimePickerModel } from "../model/timepicker/formio-timepicker.model";
import { FormioFormValueControlModel, FormioFormControlValue } from "../model/formio-form-value-control.model";

describe("FormioNg2DynamicFormService test suite", () => {

    let testModel: FormioFormControlModel[],
        service: FormioNg2DynamicFormService;

    function testValidator() {
        return {testValidator: {valid: true}};
    }

    function testAsyncValidator() {
        return new Promise<boolean>(resolve => setTimeout(() => resolve(true), 0));
    }


    beforeEach(() => {

        TestBed.configureTestingModule({
            imports: [ReactiveFormsModule],
            providers: [
                FormioNg2DynamicFormService,
                FormioNg2DynamicFormValidationService,
                {provide: NG_VALIDATORS, useValue: testValidator, multi: true},
                {provide: NG_ASYNC_VALIDATORS, useValue: testAsyncValidator, multi: true}
            ]
        });

        testModel = [

            new FormioSelectModel<string>(
                {
                    id: "testSelect",
                    options: [
                        {
                            label: "Option 1",
                            value: "option-1"
                        },
                        {
                            label: "Option 2",
                            value: "option-2"
                        }
                    ],
                    value: "option-3"
                }
            ),

            new FormioInputModel(
                {
                    id: "testInput",
                    mask: ["(", /[1-9]/, /\d/, /\d/, ")", " ", /\d/, /\d/, /\d/, "-", /\d/, /\d/, /\d/, /\d/],
                }
            ),

            new FormioCheckboxGroupModel(
                {
                    id: "testCheckboxGroup",
                    group: [
                        new FormioCheckboxModel(
                            {
                                id: "testCheckboxGroup1",
                                value: true
                            }
                        ),
                        new FormioCheckboxModel(
                            {
                                id: "testCheckboxGroup2",
                                value: true
                            }
                        )
                    ]
                }
            ),

            new FormioRadioGroupModel<string>(
                {
                    id: "testRadioGroup",
                    options: [
                        {
                            label: "Option 1",
                            value: "option-1"
                        },
                        {
                            label: "Option 2",
                            value: "option-2"
                        }
                    ],
                    value: "option-3"
                }
            ),

            new FormioTextAreaModel({id: "testTextArea"}),

            new FormioCheckboxModel({id: "testCheckbox"}),

            new FormioFormArrayModel(
                {
                    id: "testFormArray",
                    initialCount: 5,
                    groupFactory: () => {
                        return [
                            new FormioInputModel({id: "testFormArrayGroupInput"}),
                            new FormioFormArrayModel({
                                id: "testNestedFormArray", groupFactory: () => [
                                    new FormioInputModel({id: "testNestedFormArrayGroupInput"})
                                ]
                            })
                        ];
                    }
                }
            ),

            new FormioFormGroupModel(
                {
                    id: "testFormGroup",
                    group: [
                        new FormioInputModel({id: "nestedTestInput"}),
                        new FormioTextAreaModel({id: "nestedTestTextArea"})
                    ]
                }
            ),

            new FormioSliderModel({id: "testSlider"}),

            new FormioSwitchModel({id: "testSwitch"}),

            new FormioDatePickerModel({id: "testDatepicker", value: new Date()}),

            new FormioFileUploadModel({id: "testFileUpload"}),

            new FormioEditorModel({id: "testEditor"}),

            new FormioTimePickerModel({id: "testTimePicker"}),

            new FormioRatingModel({id: "testRating"})
        ];
    });


    beforeEach(inject([FormioNg2DynamicFormService], (formService: FormioNg2DynamicFormService) => service = formService));


    it("should create create a form group", () => {

        let formGroup = service.createFormGroup(testModel);

        expect(formGroup instanceof FormGroup).toBe(true);

        expect(formGroup.get("testCheckbox") instanceof FormControl).toBe(true);
        expect(formGroup.get("testCheckboxGroup") instanceof FormGroup).toBe(true);
        expect(formGroup.get("testDatepicker") instanceof FormControl).toBe(true);
        expect(formGroup.get("testFormArray") instanceof FormArray).toBe(true);
        expect(formGroup.get("testInput") instanceof FormControl).toBe(true);
        expect(formGroup.get("testRadioGroup") instanceof FormControl).toBe(true);
        expect(formGroup.get("testSelect") instanceof FormControl).toBe(true);
        expect(formGroup.get("testTextArea") instanceof FormControl).toBe(true);
        expect(formGroup.get("testFileUpload") instanceof FormControl).toBe(true);
        expect(formGroup.get("testEditor") instanceof FormControl).toBe(true);
        expect(formGroup.get("testTimePicker") instanceof FormControl).toBe(true);
        expect(formGroup.get("testRating") instanceof FormControl).toBe(true);
    });


    it("should parse dynamic form JSON", () => {

        let json = JSON.stringify(testModel),
            formModel = service.fromJSON(json);

        expect(Array.isArray(formModel) as boolean).toBe(true);

        expect(formModel[0] instanceof FormioSelectModel).toBe(true);
        expect(formModel[1] instanceof FormioInputModel).toBe(true);
        expect(formModel[2] instanceof FormioCheckboxGroupModel).toBe(true);
        expect(formModel[3] instanceof FormioRadioGroupModel).toBe(true);
        expect(formModel[4] instanceof FormioTextAreaModel).toBe(true);
        expect(formModel[5] instanceof FormioCheckboxModel).toBe(true);
        expect(formModel[6] instanceof FormioFormArrayModel).toBe(true);
        expect(formModel[7] instanceof FormioFormGroupModel).toBe(true);
        expect(formModel[8] instanceof FormioSliderModel).toBe(true);
        expect(formModel[9] instanceof FormioSwitchModel).toBe(true);
        expect(formModel[10] instanceof FormioDatePickerModel).toBe(true);
        expect((formModel[10] as FormioDateControlModel).value instanceof Date).toBe(true);
        expect(formModel[11] instanceof FormioFileUploadModel).toBe(true);
        expect(formModel[12] instanceof FormioEditorModel).toBe(true);
        expect(formModel[13] instanceof FormioTimePickerModel).toBe(true);
        expect(formModel[14] instanceof FormioRatingModel).toBe(true);
    });


    it("should throw when unknown DynamicFormControlModel id is specified in JSON", () => {

        expect(() => service.fromJSON([{id: "test"}]))
            .toThrow(new Error(`unknown form control model type defined on JSON object with id "test"`));
    });


    it("should find a dynamic form control model by id", () => {

        expect(service.findById("testCheckbox", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testCheckboxGroup", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testDatepicker", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testFormArray", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testInput", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testRadioGroup", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testSelect", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testSlider", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testSwitch", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testTextArea", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testFileUpload", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testEditor", testModel) instanceof FormioEditorModel).toBe(true);
        expect(service.findById("testTimePicker", testModel) instanceof FormioTimePickerModel).toBe(true);
        expect(service.findById("testRating", testModel) instanceof FormioRatingModel).toBe(true);
    });


    it("should find a nested dynamic form control model by id", () => {

        expect(service.findById("testCheckboxGroup1", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("testCheckboxGroup2", testModel) instanceof FormioFormControlModel).toBe(true);
        expect(service.findById("nestedTestInput", testModel) instanceof FormioFormControlModel).toBe(true);
    });


    it("should resolve array group path", () => {

        service.createFormGroup(testModel);

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            nestedModel = (model.get(0).get(1) as FormioFormArrayModel).get(0);

        expect(service.getPath(model)).toEqual(["testFormArray"]);
        expect(service.getPath(nestedModel)).toEqual(["testFormArray", "0", "testNestedFormArray", "0"]);
    });


    it("should add a form control to an existing form group", () => {

        let formGroup = service.createFormGroup(testModel),
            nestedFormGroup = formGroup.controls["testFormGroup"] as FormGroup,
            nestedFormGroupModel = testModel[7] as FormioFormGroupModel,
            newModel1 = new FormioInputModel({id: "newInput1"}),
            newModel2 = new FormioInputModel({id: "newInput2"});

        service.addFormGroupControl(formGroup, testModel, newModel1);
        service.addFormGroupControl(nestedFormGroup, nestedFormGroupModel, newModel2);

        expect(formGroup.controls[newModel1.id]).toBeTruthy();
        expect(testModel[testModel.length - 1] === newModel1).toBe(true);

        expect((formGroup.controls["testFormGroup"] as FormGroup).controls[newModel2.id]).toBeTruthy();
        expect(nestedFormGroupModel.get(nestedFormGroupModel.group.length - 1) === newModel2).toBe(true);
    });


    it("should insert a form control to an existing form group", () => {

        let formGroup = service.createFormGroup(testModel),
            nestedFormGroup = formGroup.controls["testFormGroup"] as FormGroup,
            nestedFormGroupModel = testModel[7] as FormioFormGroupModel,
            newModel1 = new FormioInputModel({id: "newInput1"}),
            newModel2 = new FormioInputModel({id: "newInput2"});

        service.insertFormGroupControl(4, formGroup, testModel, newModel1);
        service.insertFormGroupControl(0, nestedFormGroup, nestedFormGroupModel, newModel2);

        expect(formGroup.controls[newModel1.id]).toBeTruthy();
        expect(testModel[4] === newModel1).toBe(true);
        expect(service.getPath(testModel[4])).toEqual(["newInput1"]);

        expect((formGroup.controls["testFormGroup"] as FormGroup).controls[newModel2.id]).toBeTruthy();
        expect(nestedFormGroupModel.get(0) === newModel2).toBe(true);
        expect(service.getPath(nestedFormGroupModel.get(0))).toEqual(["testFormGroup", "newInput2"]);
    });


    it("should move an existing form control to a different group position", () => {

        let formGroup = service.createFormGroup(testModel),
            nestedFormGroupModel = testModel[7] as FormioFormGroupModel,
            model1 = testModel[0],
            model2 = nestedFormGroupModel.get(0);

        service.moveFormGroupControl(0, 2, testModel);

        expect(formGroup.controls[model1.id]).toBeTruthy();
        expect(testModel[2] === model1).toBe(true);

        service.moveFormGroupControl(0, 1, nestedFormGroupModel);

        expect((formGroup.controls["testFormGroup"] as FormGroup).controls[model2.id]).toBeTruthy();
        expect(nestedFormGroupModel.get(1) === model2).toBe(true);
    });


    it("should remove a form control from an existing form group", () => {

        let formGroup = service.createFormGroup(testModel),
            nestedFormGroup = formGroup.controls["testFormGroup"] as FormGroup,
            nestedFormGroupModel = testModel[7] as FormioFormGroupModel,
            length = testModel.length,
            size = nestedFormGroupModel.size(),
            index = 1,
            id1 = testModel[index].id,
            id2 = nestedFormGroupModel.get(index).id;

        service.removeFormGroupControl(index, formGroup, testModel);

        expect(Object.keys(formGroup.controls).length).toBe(length - 1);
        expect(formGroup.controls[id1]).toBeUndefined();

        expect(testModel.length).toBe(length - 1);
        expect(service.findById(id1, testModel)).toBeNull();

        service.removeFormGroupControl(index, nestedFormGroup, nestedFormGroupModel);

        expect(Object.keys(nestedFormGroup.controls).length).toBe(size - 1);
        expect(nestedFormGroup.controls[id2]).toBeUndefined();

        expect(nestedFormGroupModel.size()).toBe(size - 1);
        expect(service.findById(id2, testModel)).toBeNull();
    });


    it("should create a form array", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray;

        expect(service.createFormArray).toBeTruthy();

        formArray = service.createFormArray(model);

        expect(formArray instanceof FormArray).toBe(true);
        expect(formArray.length).toBe(model.initialCount);
    });


    it("should add a form array group", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model);

        service.addFormArrayGroup(formArray, model);

        expect(formArray.length).toBe(model.initialCount + 1);
    });


    it("should insert a form array group", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model);

        service.insertFormArrayGroup(0, formArray, model);

        expect(formArray.length).toBe(model.initialCount + 1);
    });


    it("should move up a form array group", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model),
            index = 3,
            step = 1;

        (formArray.at(index) as FormGroup).controls["testFormArrayGroupInput"].setValue("next test value 1");
        (formArray.at(index + step) as FormGroup).controls["testFormArrayGroupInput"].setValue("next test value 2");

        (model.get(index).get(0) as FormioFormValueControlModel<FormioFormControlValue>).valueUpdates.next("next test value 1");
        (model.get(index + step).get(0) as FormioFormValueControlModel<FormioFormControlValue>).valueUpdates.next("next test value 2");

        service.moveFormArrayGroup(index, step, formArray, model);

        expect(formArray.length).toBe(model.initialCount);

        expect((formArray.at(index) as FormGroup).controls["testFormArrayGroupInput"].value).toEqual("next test value 2");
        expect((formArray.at(index + step) as FormGroup).controls["testFormArrayGroupInput"].value).toEqual("next test value 1");

        expect((model.get(index).get(0) as FormioFormValueControlModel<FormioFormControlValue>).value).toEqual("next test value 2");
        expect((model.get(index + step).get(0) as FormioFormValueControlModel<FormioFormControlValue>).value).toEqual("next test value 1");
    });


    it("should move down a form array group", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model),
            index = 3,
            step = -1;

        (formArray.at(index) as FormGroup).controls["testFormArrayGroupInput"].setValue("next test value 1");
        (formArray.at(index + step) as FormGroup).controls["testFormArrayGroupInput"].setValue("next test value 2");

        (model.get(index).get(0) as FormioFormValueControlModel<FormioFormControlValue>).valueUpdates.next("next test value 1");
        (model.get(index + step).get(0) as FormioFormValueControlModel<FormioFormControlValue>).valueUpdates.next("next test value 2");

        service.moveFormArrayGroup(index, step, formArray, model);

        expect(formArray.length).toBe(model.initialCount);

        expect((formArray.at(index) as FormGroup).controls["testFormArrayGroupInput"].value).toEqual("next test value 2");
        expect((formArray.at(index + step) as FormGroup).controls["testFormArrayGroupInput"].value).toEqual("next test value 1");

        expect((model.get(index).get(0) as FormioFormValueControlModel<FormioFormControlValue>).value).toEqual("next test value 2");
        expect((model.get(index + step).get(0) as FormioFormValueControlModel<FormioFormControlValue>).value).toEqual("next test value 1");
    });


    it("should throw when form array group is to be moved out of bounds", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model);

        expect(() => service.moveFormArrayGroup(2, -5, formArray, model))
            .toThrow(new Error(`form array group cannot be moved due to index or new index being out of bounds`));
    });


    it("should remove a form array group", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model);

        service.removeFormArrayGroup(0, formArray, model);

        expect(formArray.length).toBe(model.initialCount - 1);
    });


    it("should clear a form array", () => {

        let model = service.findById("testFormArray", testModel) as FormioFormArrayModel,
            formArray = service.createFormArray(model);

        service.clearFormArray(formArray, model);

        expect(formArray.length === 0).toBe(true);
    });
});