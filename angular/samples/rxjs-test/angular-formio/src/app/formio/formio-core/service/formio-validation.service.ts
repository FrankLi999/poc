...
import {
    ValidatorFactory,
    ValidatorsToken
} from './formio-ng2-dynamic-form-validation.service';
...
@Injectable()
export class FormioValidationService {
    ...
    private getValidatorFns(validatorsConfig: FormioValidatorsMap,
        ...
        if (Utils.isTrueObject(validatorsConfig)) {
            ...
            validatorFns = Object.keys(validatorsConfig).map(validatorFnKey => {
                ...
                if (ValidationUtils.isExpandedValidatorConfig(validatorConfig)) {
                    ...
                    validatorName = (validatorConfig as FormioValidatorConfig).name;
                    validatorArgs = (validatorConfig as FormioValidatorConfig).args;
    ...
    getValidator(validatorConfig: FormioValidatorsMap): ValidatorFn | null {
    ...
    getAsyncValidator(validatorConfig: FormioValidatorsMap): AsyncValidatorFn | null {
    ...
    getValidators(validatorsConfig: FormioValidatorsMap): ValidatorFn[] {
        return this.getValidatorFns(validatorsConfig) as ValidatorFn[];
    }


    getAsyncValidators(validatorsConfig: FormioValidatorsMap): AsyncValidatorFn[] {
        return this.getValidatorFns(validatorsConfig, this.NG_ASYNC_VALIDATORS) as AsyncValidatorFn[];
    }


    createErrorMessages(control: AbstractControl, model: FormioFormControlModel): string[] {
