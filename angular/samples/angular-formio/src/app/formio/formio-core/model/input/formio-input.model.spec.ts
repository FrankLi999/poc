import {
    ...
    FORMIO_FORM_CONTROL_INPUT_TYPE_TEXT,
    FormioInputModel
} from "./formio-input.model";
import { AUTOCOMPLETE_ON } from "../../utils/autofill.utils";

describe("FormioInputModel test suite", () => {

    let model: FormioInputModel,
        config = {
            id: "input",
            list: ["One", "Two", "Three"],
            mask: ["test", /[1-9]/]
    ...
    beforeEach(() => model = new FormioInputModel(config));

    it("tests if correct default type property is set", () => {

        expect(model.type).toEqual(FORMIO_FORM_CONTROL_TYPE_INPUT);
    });

    it("tests if correct default input type property is set", () => {

        expect(model.inputType).toEqual(FORMIO_FORM_CONTROL_INPUT_TYPE_TEXT);
    });
    ...
    it("should serialize correctly", () => {
        ...
        expect(json.type).toEqual(FORMIO_FORM_CONTROL_TYPE_INPUT);
    });
});