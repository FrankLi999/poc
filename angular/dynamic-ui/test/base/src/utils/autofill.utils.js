...
var AutoFillUtils = (function () {
    ...
    AutoFillUtils.isAddressToken = function (token) {
        return exports.AUTOFILL_TOKENS_ADDRESS.indexOf(token) > -1;
    };
    AutoFillUtils.isContactField = function (token) {
        return exports.AUTOFILL_FIELDS_CONTACT.indexOf(token) > -1;
    };
    AutoFillUtils.isContactToken = function (token) {
        return exports.AUTOFILL_TOKENS_CONTACT.indexOf(token) > -1;
    };
    AutoFillUtils.isField = function (token) {
        return exports.AUTOFILL_FIELDS.indexOf(token) > -1;
    };
    AutoFillUtils.isSectionToken = function (token) {
        return token.startsWith("section-");
    };
    AutoFillUtils.validate = function (tokens) {
        var toExpression = function (total, currentValue) { return total + "|" + currentValue; }, tokensAddress = exports.AUTOFILL_TOKENS_ADDRESS.reduce(toExpression), tokensContact = exports.AUTOFILL_TOKENS_CONTACT.reduce(toExpression), fields = exports.AUTOFILL_FIELDS.reduce(toExpression), fieldsContact = exports.AUTOFILL_FIELDS_CONTACT.reduce(toExpression), regex = new RegExp("^(section-\\w+\\s{1})?((" + tokensAddress + "){1}\\s)?((" + fields + "){1}|((" + tokensContact + "){1}\\s{1}(" + fieldsContact + ")))$");
        return regex.test(tokens);
    };
    return AutoFillUtils;
}());
exports.AutoFillUtils = AutoFillUtils;
