...
import algo.functional.LinkedList;
import algo.structure.OrderedPair;
...
public class ArraySorter {
	...
	public static <E extends Comparable<E>> void insertElementSortedNonRecursive(E[] array, int valueIndex) {
		while (true) {
			if (valueIndex > 0 && array[valueIndex].compareTo(array[valueIndex - 1]) < 0) {
				swap(array, valueIndex, valueIndex - 1);
...
//let us find the pivot.
	...
	public static <E> OrderedPair<LinkedList<E>, LinkedList<E>> splitByPivot(LinkedList<E> input, E pivot,
			Comparator<E> comparator) {
		if (input.isEmpty()) {
			return new OrderedPair<>(input, input);
		} else {
			OrderedPair<LinkedList<E>, LinkedList<E>> subSplit = splitByPivot(input.tail(), pivot, comparator);
			if (comparator.compare(pivot, input.head()) > 0) {
				return new OrderedPair<>(subSplit._1.add(input.head()), subSplit._2);
			} else {
				return new OrderedPair<>(subSplit._1, subSplit._2.add(input.head()));
			}
		}

	}

	public static <E> LinkedList<E> quicksort(LinkedList<E> input, Comparator<E> comparator) {
		if (input.isEmpty()) {
			return input;
		} else {
			E pivot = input.head();
			OrderedPair<LinkedList<E>, LinkedList<E>> split = splitByPivot(input.tail(), pivot, comparator);
	...
	public static <E> OrderedPair<LinkedList<E>, LinkedList<E>> splitAtIndex(LinkedList<E> input, int index) {
		if (input.isEmpty()) {
			...
			OrderedPair<LinkedList<E>, LinkedList<E>> subSplit = splitAtIndex(input.tail(), index - 1);
			return new OrderedPair<>(subSplit._1.add(input.head()), subSplit._2);
		}
	}
	...
	public static <E> LinkedList<E> mergesort(LinkedList<E> list, Comparator<E> comparator) {
		if (list.isEmpty() || list.getLength() == 1) {
			...
			OrderedPair<LinkedList<E>, LinkedList<E>> split = splitAtIndex(list, list.getLength() / 2);
			LinkedList<E> sortedPart1 = mergesort(split._1, comparator);
			LinkedList<E> sortedPart2 = mergesort(split._2, comparator);
			return merge(sortedPart1, sortedPart2, comparator);
		}
	}

	private static <E> void merge(E[] arrayL, E[] arrayR, int start, int mid, int end, E[] targetArray,
			Comparator<E> comparator) {
		int i = start;
		int j = mid;
		int k = start;
	...
	private static <E> void mergeRecursive(E[] arrayL, E[] arrayR, int startL, int endL, int startR, int endR,
			E[] targetArray, int targetPosition, Comparator<E> comparator) {
		if (startL == endL && startR == endR) {
			return;
		} else if (startR == endR) {
			targetArray[targetPosition] = arrayL[startL];
			mergeRecursive(arrayL, arrayR, startL + 1, endL, startR, endR, targetArray, targetPosition + 1, comparator);
		} else if (startL == endL) {
			targetArray[targetPosition] = arrayR[startR];
			mergeRecursive(arrayL, arrayR, startL, endL, startR + 1, endR, targetArray, targetPosition + 1, comparator);
		} else if (comparator.compare(arrayL[startL], arrayR[startR]) > 0) {
			targetArray[targetPosition] = arrayR[startR];
			mergeRecursive(arrayL, arrayR, startL, endL, startR + 1, endR, targetArray, targetPosition + 1, comparator);
		} else {
	...
	public static <E> E[] mergeSortNoCopy(E[] sourceArray, int start, int end, E[] tempArray,
			Comparator<E> comparator) {
		if (start >= end - 1) {
			return sourceArray;
		}
...
//in this case we store the result in sortedPart2 because it has the first portion empty
	...
	private static <E> LinkedList<E> merge(LinkedList<E> list1, int length1, LinkedList<E> list2,
			Comparator<E> comparator) {
		if (length1 == 0) {
			return list2;
			...
			if (comparator.compare(l, r) > 0) {
				return merge(list1, length1, list2.tail(), comparator).add(r);
			} else {
				return merge(list1.tail(), length1 - 1, list2, comparator).add(l);
			}
		}
	}
	...
	public static void main(String[] args) {
        ...
        //quicksort(array1, (a, b) -> a - b);
        Integer[] anotherArray = new Integer[array1.length];
//        mergeSort(array1, 0, array.length, anotherArray,
//                (a, b) -> a - b);
//        array = mergeSortNoCopy(array1, 0, array.length, anotherArray,
//                (a, b) -> a - b);
