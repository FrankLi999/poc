...
import algo.structure.DoublyLinkedList;
import algo.structure.LinkedList;
import algo.tree.BinarySearchTree;
import algo.tree.RedBlackTree;
...
public class AdjacencyListGraphWithDenseVertex<V,E> implements Graph<V,E> {
    ...
    public void setEdgeValue(int source, int target, E value) {
        ...
        if(sNode == null){
            ...
            if(edgeNode!=null) {
                ...
                throw new IllegalArgumentException("No edge exists between the vertices "
                        + source + " and " + target);
            }
        }
    }
    ...
    public static void main(String [] args){
        AdjacencyListGraphWithDenseVertex<Character, Integer> graph = new AdjacencyListGraphWithDenseVertex<>(true);
        for(int i=0;i<10;i++){
            graph.addVertex();
        }
...
//        graph.getNeighbors(2).forEach(System.out::println);
    ...
    //    graph.removeVertex(3);
        ...
        graph.visitAllConnectedEdges(0, (s, t, e)-> System.out.println("" +s+ " -> "+t + " = "+e), TraversalType.DFT);
        graph.visitAllConnectedVertices(0, (id, value)-> System.out.print(" "+ id + "=" + value), TraversalType.BFT);
        System.out.println();
//        graph.spanningTree(0).forEach((e)-> System.out.println(""+e.source+" -> "+e.target));
        System.out.println(graph.detectCycle());
        graph.minimumSpanningTree((v)->v).forEach((e)->System.out.println("** "+e.source + " -> "+ e.target));
    }
}
