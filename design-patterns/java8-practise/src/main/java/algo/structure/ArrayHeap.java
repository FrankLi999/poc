package algo.structure;

import java.util.Arrays;
import java.util.Comparator;
import java.util.Iterator;

import algo.functional.NoSpaceException;
...
public class ArrayHeap<E> implements PriorityQueue<E>{
    ...
    protected void trickleDown(int position){
        ...
        if(rightChild<numElements) {
            if (comparator
                    .compare(store[leftChild], store[rightChild]) < 0) {
                if (comparator
                        .compare(store[leftChild], store[position]) < 0) {
                    swap(position, leftChild);
                    trickleDown(leftChild);
                }
            } else {
                if (comparator
                        .compare(store[rightChild], store[position]) < 0) {
    ...
    public Iterator<E> iterator() {
        return new Iterator<E>() {
            ...
            public boolean hasNext() {
                return nextPosition<store.length;
            }
